...
<html lang="en">
<head>
  <title>Brand Reports({{ date("d M Y") }})</title>
        ...
        <table>
          ...
          <tbody>
            ...
            <tr>
              <td>{{ $serial++; }}</td>
              <td>{{ $status->name }}</td>
              <td><img id="zoom" src="{{ url('public/image/brand/') }}/{{ $status->logo }}" width="150" height="100"></td>
              <td>{{ $status->total_product }}</td>
            </tr>
            @endforeach
          </tbody>
        </table>
...
<script>
  function download(){
    window.location.href = "{{url('brandReportDownload')}}";
  }
  function home(){
